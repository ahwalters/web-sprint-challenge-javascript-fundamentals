...
//The nested function can access the variable internal because it is declared inside the same function as the nested function. They are in the same scope.
...
//originally i'd returned sum within the loop, why would that give 0????????
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(animalsArr){
    const newArr = []
    animalsArr.forEach((element) => {newArr.push(`name: ${element.animal_name}, scientific: ${element.scientific_name}`)})
    return newArr; 
  };
  ...
  */
  ...
  function lowerCaseNames(animalsArr){
    const newArr = animalsArr.map(element => element.animal_name.toLowerCase());
    return newArr;
  }
  ...
  */
  ...
  function lowPopulationAnimals(animalsArr){
    const filteredArr = animalsArr.filter(function (element) {
  ...
  */
  ...
  function USApop(animalsArr){
    const population = animalsArr.reduce((accumulator, currentValue) => {
...
const myCuboid = new CuboidMaker({length: 4, width: 5, height: 5})
